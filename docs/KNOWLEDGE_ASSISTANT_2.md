...
3. **异步摄取体验与生产化**：2.3 已有本机 SQLite 单 worker、取消和恢复语义；2.4-A 已将选择/确认阶段的 SHA-256 预检异步化并显示进度与取消；2.4-C 首切片已支持文件拖放并走同一预检路径；生产仍需对象存储、恶意文件扫描、沙箱解析、外部队列、多节点 fencing 与容量压测。
