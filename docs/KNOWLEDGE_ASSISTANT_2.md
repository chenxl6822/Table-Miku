...
6. **2.5 可用任务工具（进行中）**：任务页已提供「待我审批」收件箱、到期提示，以及 Approver/Admin 跨页的仅计数收件箱通知。窗口刷新后收件箱数量上升时，管理台会通过主窗口系统托盘弹出同样仅含计数的气泡；点击气泡打开收件箱。没有托盘或不支持气泡时静默跳过，不新增关闭窗口后的轮询。Editor 可创建 `create_work_item` 审批任务，批准后写入本机 SQLite 工作项账本；也可用 `close_work_item` 经独立审批把账本行标为 `closed`（不删行、不发 HTTP）。远端幂等键与 HTTP `Idempotency-Key` 分离。真实 HTTP 外部工具仍待后续切片。
