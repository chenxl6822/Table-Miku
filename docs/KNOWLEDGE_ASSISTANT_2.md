...
4. **2.4-B 检索质量闭环（下一阶段）**：引入真实语义 embedding、reranker、逐句证据、反馈和知识缺口；是否上线由业务金标集与任务完成率共同决定。
...
3. **异步摄取体验与生产化**：2.3 已有本机 SQLite 单 worker、取消和恢复语义；2.4-A 已将选择/确认阶段的 SHA-256 预检异步化并显示进度与取消，同时保留发送 worker 的最终哈希复核；生产仍需对象存储、恶意文件扫描、沙箱解析、外部队列、多节点 fencing 与容量压测。
