...
桌面端“批量添加资料”先给用户展示精确写入范围：文件名、规范化路径、字节数、目标集合和 SHA-256。确认快照包含规范路径、大小、纳秒修改时间、设备/文件标识和完整 SHA-256；发送工作线程会从打开的文件句柄重新校验这些字段，并对实际读取字节再次计算 SHA-256。文件在确认后被替换、截断或以相同大小改写都会失败关闭，不会把未确认的新内容发送到服务端。
...
| 文件替换/确认后改写 | 选择时从同一句柄建立路径、文件标识、大小、时间与 SHA-256 快照；发送前重读并核对实际字节 | 仍不是恶意文件扫描；远程/特殊文件系统语义需单独验证 |
...
3. **2.4 检索质量闭环（下一阶段）**：先把批量文件哈希预检移到可取消的后台流程，避免慢盘/网络盘冻结确认窗口；随后引入真实语义 embedding、reranker、逐句证据、反馈和知识缺口；是否上线由业务金标集与任务完成率共同决定。
...
3. **异步摄取体验与生产化**：2.3 已有本机 SQLite 单 worker、取消和恢复语义；下一小步先异步执行选择/确认阶段的 SHA-256 预检并显示进度与取消，同时保留发送 worker 的最终哈希复核；生产仍需对象存储、恶意文件扫描、沙箱解析、外部队列、多节点 fencing 与容量压测。
