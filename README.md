...
## Knowledge Assistant 2.x

仓库新增了一个与桌面端数据隔离、可独立部署的企业知识库与任务处理纵向切片，覆盖文件摄取、本地向量检索、来源引用、无答案拒答、工具任务、写审批、RBAC、幂等收据、链路追踪与离线质量门。

- 设计、API、权限、威胁模型、部署边界和验收证据见 [Knowledge Assistant 2.3 实施与运维手册](docs/KNOWLEDGE_ASSISTANT_2.md)。
...
管理台提供文档、RAG 引用/拒答、任务审批、操作收据、摄取中心、延迟和 Token 面板。审批页把可信动作契约与不可信 Agent 正文分区显示，正文只按纯文本渲染；身份字段变化、身份切换、暂缓或关闭会清除当前审批状态，必须重新加载预览。带集合限制的身份访问租户级 metrics 或 Trace 时由服务端返回 403，界面禁用只是提前提示。界面中的租户、用户和角色切换只用于本地 UAT；它不会替代生产登录和可信网关。

2.2 管理台优先改善普通用户的首次使用和核查体验：默认进入 RAG 查询页，顶部说明“上传资料 → 提问 → 核查引用”；本地验收身份设置默认折叠，并持续显示当前角色能做什么。RAG 答案支持标题、列表、表格、强调和代码块，但禁用 HTML、图片、外部资源和链接跳转；引用行可打开纯文本证据详情。任务主视图改为人类可读的动作、目标、状态进度和操作收据，原始 JSON 收入可折叠技术详情；审批按钮会解释禁用原因。审批正文仍逐字按纯文本显示，不能用 Markdown 美化视图代替精确 Action Preview。

2.3 把批量文档摄取移到后台：Editor 一次最多选择 20 个文件，确认界面展示文件、大小、集合和 SHA-256 摘要；工作线程会在发送前重新核对规范路径、文件元数据和完整 SHA-256。服务端把任务、原始负载、真实处理阶段、错误、Trace 和取消结果持久化；“取消请求中”只表示已提出请求，只有 `cancelled` 才表示已取消，若提交已先完成则会显示“取消过晚，文档已写入”。“摄取中心”用于查看进度、失败、待确认结果和恢复记录。

Windows 桌面端会在网络写入前用当前 Windows 用户的 DPAPI 加密保存精确请求；结果不确定时不会自动重放，也不会自动换幂等键。恢复记录同时绑定原身份与稳定服务实例，外部模式还绑定规范化服务 origin；用户核对后才能“安全重试原请求”或明确放弃。该 outbox 最多保留 200 条、占用 256 MiB；DPAPI 不可用、服务实例标识缺失或记录不匹配时失败关闭。

首次本地验收可按以下路径进行：

1. 默认 Viewer 直接提问；如果还没有资料，前往“文档”，展开本地验收身份设置，选择 Editor 并应用后使用“批量添加资料”，再到“摄取中心”核对处理结果。
2. 回到“RAG 查询”，提问并选择引用，核对文件、集合、位置、相关度和索引原文摘录。
3. Editor 创建写入任务后，任务会显示“等待另一位审批人”；请求人不能批准自己的任务。
4. 改用不同用户名的 Approver，加载与该审批人绑定的精确 Action Preview，再批准、拒绝或安全暂缓。

2.3 只把批量文档摄取移到后台；文件选择和最终确认仍会在界面线程中计算 SHA-256，慢盘或网络盘可能短暂阻塞确认窗口，2.4 将把该预检改为可取消的后台流程。RAG、任务与审批等管理台 HTTP 调用也仍可能在 UI 线程同步执行。服务端仍是 SQLite 单节点、单摄取 worker，并使用依赖无关的本地哈希向量；PDF 只有文本提取，没有 OCR、恶意文件扫描或解析沙箱。outbox 仅支持 Windows DPAPI，未知网络结果不自动重放，普通解析失败也不自动重试。本阶段未部署到目标服务器，当前能力适合本地演示、开发与安全边界验证，不代表已经满足大规模生产容量、安全或语义质量要求。
